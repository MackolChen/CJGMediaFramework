 ...
 */
- (void)setPlayerLayer{
    if (_playerType == CJGAVPlayerTypeAudio) return;
    //视频播放才有视频预览层
    ...
    switch (status) {
        case AVPlayerItemStatusReadyToPlay: {
            ...
            if (self.delegate && [self.delegate respondsToSelector:@selector(didChangedPlayState:player:)]) {
                [self.delegate didChangedPlayState:CJGAVPlayerStateReadyToPlay player:self];
            }
        ...
        case AVPlayerItemStatusFailed: {
            ...
            _isPlaying = NO;
            if (self.delegate && [self.delegate respondsToSelector:@selector(didChangedPlayState:player:)]) {
                [self.delegate didChangedPlayState:CJGAVPlayerStateFailed player:self];
            }
            [self stop];
        }
            break;
        ...
        case AVPlayerItemStatusUnknown: {
            ...
            _isPlaying = NO;
            if (self.delegate && [self.delegate respondsToSelector:@selector(didChangedPlayState:player:)]) {
                [self.delegate didChangedPlayState:CJGAVPlayerStateUnKnow player:self];
            }
        }
            break;
...
- (void)play{
    ...
    [_player play];
    if (self.delegate && [self.delegate respondsToSelector:@selector(didChangedPlayState:player:)]) {
        [self.delegate didChangedPlayState:CJGAVPlayerStatePlaying player:self];
    }
}
...
- (void)pause {
    ...
    [_player pause];
    if (self.delegate && [self.delegate respondsToSelector:@selector(didChangedPlayState:player:)]) {
        [self.delegate didChangedPlayState:CJGAVPlayerStatePause player:self];
    }
}
...
- (void)stop {
    ...
    [_player replaceCurrentItemWithPlayerItem:_playerItem];
    if (self.delegate && [self.delegate respondsToSelector:@selector(didChangedPlayState:player:)]) {
        [self.delegate didChangedPlayState:CJGAVPlayerStateFinish player:self];
    }
}
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSKeyValueChangeKey,id> *)change context:(void *)context {
    ...
    if (object == _playerItem) {
        ...
        if ([keyPath isEqualToString:@"status"]) {
            ...
            if (self.delegate && [self.delegate respondsToSelector:@selector(didChangedPlayState:player:)]) {
                [self.delegate didChangedPlayState:_isBuffering?CJGAVPlayerStateBuffering:CJGAVPlayerStateBufferFinish player:self];
            }
        }
    }
}
