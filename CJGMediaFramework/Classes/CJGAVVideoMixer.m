...
@interface CJGAVVideoMixer ()<CJGAVAssetExportSessionDelegate>
{
    // 混合的composition
    ...
    // 回调
    void (^_handler)(NSString *filePath, CJGAVMixStatus status);
    // 是否需要手动调用start
...
// 校验视频时间范围
@property (nonatomic, strong) CJGAVTimeRange *videoTimeRange;
...
@implementation CJGAVVideoMixer

#pragma mark - setter getter
- (void)setMixAudios:(NSArray<CJGAVAudioMixerOptions *> *)mixAudios;
{
    _mixAudios = mixAudios;
}

- (void)setMainVideo:(CJGAVVideoMixerOptions *)mainVideo{
 ...
 */
- (instancetype)initWithMixerVideoOptions:(CJGAVVideoMixerOptions *)mainVideo;
{
    if (self = [super init]) {
 ...
 */
- (void)startMixingVideoWithCompletion:(void (^ _Nullable)(NSString* , CJGAVMixStatus))handler;
{
    _needStartCompressing = NO;
    if (_mainVideo.mixStatus == CJGAVMixStatusMixing) {
        [self cancelMixing];
        _handler = handler;
    ...
    if (!_mainVideo) {
        CJGLError(@"have not set a valid main track");
        [self notifyStatus:CJGAVMixStatusCancelled];
        return;
    }
...
- (void)processVideoOfAudioMixWithCompletion:(void (^ _Nullable)(NSString *mixAudioFilePath))handler;
{
    ...
    if (_mixAudios && _mixAudios.count > 0) {// 保留视频原音
        ...
        if (_mainVideo.enableVideoSound) {//保留视频原音
            ...
            CJGAVAudioMixerOptions *mainAudio;
            if (audioTracks.count > 0) {
                
                AVAssetTrack *mainAudioTrack = [audioTracks firstObject];
                mainAudio = [[CJGAVAudioMixerOptions alloc] init];
                ...
                CJGLWarn(@"This video has no audio tracks.");
                mainAudio = [[CJGAVAudioMixerOptions alloc]init];
            ...
            CJGAVAudioMixerOptions *mainAudio = [[CJGAVAudioMixerOptions alloc]init];
            mainAudio.enableCycleAdd = NO;
            mainAudio.atTimeRange = _videoTimeRange;
            mainAudio.audioVolume = 0;
 ...
 */
- (void)mixAudioWithMainTrack:(CJGAVAudioMixerOptions *)mainAudio completionHandler:(void (^)(NSString *filePath))handler;
{
    CJGAVAudioMixer *audioMix = [[CJGAVAudioMixer alloc]init];
    audioMix.mainAudio = mainAudio;
    audioMix.mixAudios = _mixAudios;
    
    [audioMix startMixingAudioWithCompletion:^(NSString *filePath, CJGAVMixStatus status) {
        if (status == CJGAVMixStatusCompleted) {
            if (handler) {
                handler(filePath);
            }
        }else if(status == CJGAVMixStatusFailed){
            
            CJGLError(@"Audio mixing failure");
            [self notifyStatus:CJGAVMixStatusFailed];
        }
    }];
...
/**
 ...
 @param handler 回调
 */
- (void)exportVideoWithMixAudioPath:(NSString *)mixAudioPath CompletionHandler:(void (^)(NSString* outputFilePath, CJGAVMixStatus status))handler;
{
    ...
    if (mixAudioPath) {
        ...
        if (error) {
            CJGLError(@"compositionAudioTrack insert error : %@",error);
        }
    ...
    if (error) {
        CJGLError(@"_compositionVideoTrack insert error : %@",error);
    }
    ...
    //正在合成通知回调
    [self notifyStatus:CJGAVMixStatusMixing];
    ...
    [_exporter exportAsynchronouslyWithCompletionHandler:^{
        ...
        CJGAVMixStatus exportStatus = CJGAVMixStatusUnknown;
        switch (self->_exporter.status)
        {
            case AVAssetExportSessionStatusUnknown:
                exportStatus = CJGAVMixStatusUnknown;
                break;
            case AVAssetExportSessionStatusExporting:
                exportStatus = CJGAVMixStatusMixing;
                break;
            case AVAssetExportSessionStatusCompleted:
                exportStatus = CJGAVMixStatusCompleted;
                break;
            case AVAssetExportSessionStatusFailed:
                exportStatus = CJGAVMixStatusFailed;
                break;
            case AVAssetExportSessionStatusCancelled:
                exportStatus = CJGAVMixStatusCancelled;
                break;
            default:
                exportStatus = CJGAVMixStatusFailed;
                break;
 ...
 */
- (void)notifyStatus:(CJGAVMixStatus)status;
{
...
#pragma mark - CJGAVAssetExportSessionDelegate
- (void)exportSession:(CJGAVAssetExportSession *)exportSession notifyStatus:(AVAssetExportSessionStatus)status;
{
