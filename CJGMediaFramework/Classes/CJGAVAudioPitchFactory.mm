 ...
 * 音频变调器工厂
 */
@interface CJGAVAudioPitchFactory (){
    /** 是否已被释放 */
    BOOL _released;
    /** 音频变调接口 */
    CJGAVComponent::AudioPitch *_audioPitch;
    /** 音频描述信息 */
...
@implementation CJGAVAudioPitchFactory
...
- (void)buildAudioPitch;
{
    ...
    //设置音频信息赋值
    CJGAVComponent::AudioInfo info = {(uint8_t)_inputInfo.channelsPerFrame,bitWidth,(uint32_t)_inputInfo.sampleRate};
    //设置Apple媒体监听信息
    CJGAVComponent::MediaAppleListener *listenner = new CJGAVComponent::MediaAppleListener(^(CJGAVComponent::AppleAudioData *data){
       
        if (!self->_mediaSync) return;
        //通过处理得到的音频数据重新封包生成CMSampleBufferRef
        CMSampleBufferRef sampleBuffer = [CJGAVMediaSampleBufferAssistant createAudioSample:data->ptr length:(int)data->info.size time:(data->info.timeUs - self->_startTimeUS) audioStreamBasicDescription:self->_asbd];
        BOOL autoRelese = NO;
        //同步音频重变调后数据
    ...
    //初始化音频变调器
    _audioPitch = new CJGAVComponent::AudioPitch(info);
    //初始化媒体监听对象
 ...
 */
- (void)setInputInfo:(CJGAVAudioTrackInfo *)inputInfo;
{
    if (!inputInfo) {
        CJGLWarn(@"AudioPitch changeFormat need inputInfo.");
        return;
    }
...
- (BOOL)queueInputBuffer:(CMSampleBufferRef)inputBuffer;
{
    if (!inputBuffer)
    {
        CJGLWarn(@"AudioPitch queueInputBuffer need inputBuffer.");
        return NO;
    }
   
    //初始化Apple音频数据
    CJGAVComponent::AppleAudioData *data = new CJGAVComponent::AppleAudioData();
    //声明BufferInfo缓存信息
    CJGAVComponent::BufferInfo bufferInfo;
    ...
    //媒体缓存定义
    CJGAVComponent::TBuffer mBuffer = make_shared<CJGAVComponent::MediaBufferApple>(data);
