//
//  CJGAVMediaTimelineSliceComposition.m
//  CJGAVComponent
//
...
@interface CJGAVMediaTimelineSliceComposition()<CJGAVAssetExportSessionDelegate>
{
    // 混合的composition
    AVMutableComposition    *_mixComposition;
    // 输出的 session
    CJGAVAssetExportSession *_exporter;
    // 音视频 asset
...
@implementation CJGAVMediaTimelineSliceComposition
...
- (void)cancelMediaComposition;
{
    ...
    [self notifyStatus:CJGAVMediaTimelineSliceCompositionStatusCancelled];
...
- (void)startVideoCompositionWithCompletionHandler:(void (^ _Nullable)(NSString *outputFilePath,NSTimeInterval mediaTotalTime, CJGAVMediaTimelineSliceCompositionStatus status))handler;
{
    ...
    if (_timeSliceOptions.enableVideoSound) {
        ...
        if (audioTrackArr.count > 0) {
            ...
            CJGLError(@"This mediaAsset has no audio tracks，Please check if the mediaAsset is correct");
            return;
        }
    }
    ...
    if (videoTrackArr.count > 0) {
        ...
        CJGLError(@"This mediaAsset has no video tracks，Please check if the mediaAsset is correct");
        return ;
    }
}
...
/**
 ...
 @param handler 回调
 */
- (void)exportVideoWithVideoTrack:(AVMutableCompositionTrack *)videoTrack assetVideoTrack:(AVAssetTrack *)assetVideoTrack completionHandler:(void (^)(NSString* outputFilePath,NSTimeInterval mediaTotalTime,CJGAVMediaTimelineSliceCompositionStatus status))handler;
{
    ...
    if (!_exporter) {
        ...
        _exporter = [[CJGAVAssetExportSession alloc] initWithAsset:_mixComposition];
    ...
    //正在合成通知回调
    [self notifyStatus:CJGAVMediaTimelineSliceCompositionStatusComposing];

    //导出视频素材
    [_exporter exportAsynchronouslyWithCompletionHandler:^{
        
        if (self->_exporter.error)
            CJGLError(@"exporter error :  %@",self->_exporter.error);
        
        CJGAVMediaTimelineSliceCompositionStatus exportStatus = CJGAVMediaTimelineSliceCompositionStatusFailed;
        switch (self->_exporter.status)
        {
            case AVAssetExportSessionStatusUnknown:
                exportStatus = CJGAVMediaTimelineSliceCompositionStatusFailed;
                break;
            case AVAssetExportSessionStatusExporting:
                exportStatus = CJGAVMediaTimelineSliceCompositionStatusComposing;
                break;
            case AVAssetExportSessionStatusCompleted:
                exportStatus = CJGAVMediaTimelineSliceCompositionStatusCompleted;
                break;
            case AVAssetExportSessionStatusFailed:
                exportStatus = CJGAVMediaTimelineSliceCompositionStatusFailed;
                break;
            case AVAssetExportSessionStatusCancelled:
                exportStatus = CJGAVMediaTimelineSliceCompositionStatusCancelled;
                break;
            default:
                exportStatus = CJGAVMediaTimelineSliceCompositionStatusFailed;
                break;
 ...
 * @param handler block
 */
- (void)exportAudioWithAudioTrack:(AVMutableCompositionTrack *)compositionAudioTrack assetAudioTrack:(AVAssetTrack *)assetAudioTrack completionHandler:(void (^)(NSString* outputFilePath,NSTimeInterval mediaTotalTime, CJGAVMediaTimelineSliceCompositionStatus status))handler;
{
    //初始化导出素材对象导出器
    if (!_exporter) {
...
- (void)resetCompositionOperation;
{
    if (_exporter.status == AVAssetExportSessionStatusExporting || _exporter.status == AVAssetExportSessionStatusWaiting) {
        CJGLDebug(@"Conditions cannot be reset during operation.");
...
- (void)judgeAssetIsVaild{
    ...
    if (!_timeSliceOptions.mediaPath) {
        CJGLError(@"mediaPath is nil, please choose a video file to mix");
        //合成失败状态回调
        [self notifyStatus:CJGAVMediaTimelineSliceCompositionStatusFailed];
        return;
    }
    ...
    //开始合成状态回调
    [self notifyStatus:CJGAVMediaTimelineSliceCompositionStatusStart];
}
...
- (CMTime)adjustMediaTimeSliceWith:(AVMutableCompositionTrack *)compositionTrack;
{
    ...
    //遍历时间片段数组，将每个时间切片c取出
    for (CJGAVMeidaTimelineSlice *timeSlice in _meidaFragmentArr) {
        // 未变速前的对应持续时间
...
/**
 ...
 @return 新的duration
 */
- (CMTime)getNewDurationWithDuration:(CMTime)duration WithSpeedMode:(CJGAVRecordSpeedMode)speedMode;
{
    CGFloat scale = 1.0;
    switch (speedMode) {
 ...
 */
- (void)notifyStatus:(CJGAVMediaTimelineSliceCompositionStatus)status;
{
    //将最后媒体素材的时间存入
    ...
    CJGRunSynchronouslyOnMainQueue(^{
        ...
        if (status == CJGAVMediaTimelineSliceCompositionStatusCompleted) {
...
- (void)exportSession:(CJGAVAssetExportSession *)exportSession progress:(CGFloat)progress{
    
    [self notifyProgress:progress];
    CJGLDebug(@"----->%f",progress);
}
@end
