...
@class CJGAVAssetExportSession;

@protocol CJGAVAssetExportSessionDelegate <NSObject>
@optional
- (void)exportSession:(CJGAVAssetExportSession *)exportSession renderFrame:(CVPixelBufferRef)pixelBuffer withPresentationTime:(CMTime)presentationTime toBuffer:(CVPixelBufferRef)renderBuffer;
// 目前只对cancel 进行回调
 ...
 */
@interface CJGAVAssetExportSession : NSObject

@property (nonatomic, weak) id<CJGAVAssetExportSessionDelegate> delegate;
