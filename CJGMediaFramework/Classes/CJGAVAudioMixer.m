...
@implementation CJGAVAudioMixer
...
- (void)setMainAudio:(CJGAVAudioMixerOptions *)mainAudio;
{
...
- (void)startMixingAudioWithCompletion:(void (^ _Nullable)(NSString*, CJGAVMixStatus))handler;
{
    ...
    if (_mixAudios && _mixAudios.count > 0) {
        [_mixAudios enumerateObjectsUsingBlock:^(CJGAVMixerOptions * _Nonnull audio, NSUInteger idx, BOOL * _Nonnull stop) {
            //如果音轨为空，跳出遍历
 ...
 */
- (void)addAudioTrack:(CJGAVMixerOptions *)audioOptions atTimeRange:(CJGAVTimeRange *)timeRange mainTimeRange:(CJGAVTimeRange *)mainTimeRange;
{
    ...
    if (!audioOptions.enableCycleAdd || CMTIME_COMPARE_INLINE(atTimeRange.duration, >, mainTimeRange.duration)) {//不需要循环播放
        ...
        while (CMTIME_COMPARE_INLINE(nextAtTime, <=, audioDurationTime)) {
            ...
            if (!insertResult) {
                CJGLError(@"enableCycleAdd mix insert error 1: %@",error);
                break;
            }
 ...
 * @param handler block
 */
- (void)exportAudioWithCompletionHandler:(void (^ _Nullable)(NSString*, CJGAVMixStatus))handler;
{
    ...
    [_exporter exportAsynchronouslyWithCompletionHandler:^{
        ...
        if (self.exporter.error) {
            CJGLError(@"exporter audio error : %@",self.exporter.error);
        }
