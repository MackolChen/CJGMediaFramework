...
- (void)loadAsynchronouslyForAssetInfo:(AVAsset *)asset completionHandler:(void (^)(void))handler;
{
    ...
    NSMutableArray<CJGAVAudioTrackInfo *> *audioTrackInfoArray = [NSMutableArray arrayWithCapacity:1];
    ...
    [asset loadValuesAsynchronouslyForKeys:@[@"tracks",@"duration"] completionHandler: ^{
        ...
        dispatch_sync(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            ...
            if (trackStatus != AVKeyValueStatusLoaded || error) {
                CJGLError(@"audioTrack loadValuesAsynchronouslyForKeys is failed : %@",error);
                if (handler) handler();
                return ;
            }
            ...
            //遍历音轨数组来进行CJGAVAudioTrackInfo值入
            [audioTracks enumerateObjectsUsingBlock:^(AVAssetTrack * audioTrack, NSUInteger idx, BOOL * _Nonnull stop) {
                ...
                CJGAVAudioTrackInfo *audioTrackInfo = [[CJGAVAudioTrackInfo alloc] initWithCMAudioFormatDescriptionRef:audioFormatDescriptionRef];
...
- (NSString *)description{
    ...
    [_audioTrackInfoArray enumerateObjectsUsingBlock:^(CJGAVAudioTrackInfo * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
@implementation CJGAVAudioTrackInfo
