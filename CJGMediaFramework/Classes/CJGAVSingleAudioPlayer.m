...
@interface CJGAVSingleAudioPlayer ()<AVAudioPlayerDelegate>
{
...
@implementation CJGAVSingleAudioPlayer

static CJGAVSingleAudioPlayer *player = nil;
...
- (void)play{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(didChangedAudioPlayState:player:)]) {
        [self.delegate didChangedAudioPlayState:CJGAVPlayerStatePlaying player:self];
    }
...
- (void)pause{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(didChangedAudioPlayState:player:)]) {
        [self.delegate didChangedAudioPlayState:CJGAVPlayerStatePause player:self];
    }
...
- (void)stop{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(didChangedAudioPlayState:player:)]) {
        [self.delegate didChangedAudioPlayState:CJGAVPlayerStateStop player:self];
    }
...
- (void)audioPlayerDidFinishPlaying:(AVAudioPlayer *)player successfully:(BOOL)flag{
    ...
    if (flag) {
        if (self.delegate && [self.delegate respondsToSelector:@selector(didChangedAudioPlayState:player:)]) {
            [self.delegate didChangedAudioPlayState:CJGAVPlayerStateFinish player:self];
        }
    }else{
        if (self.delegate && [self.delegate respondsToSelector:@selector(didChangedAudioPlayState:player:)]) {
            [self.delegate didChangedAudioPlayState:CJGAVPlayerStateFailed player:self];
        }
    }
