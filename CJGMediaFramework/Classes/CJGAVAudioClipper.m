...
@implementation CJGAVAudioClipper

#pragma mark - setter getter

- (void)setClipAudio:(CJGAVAudioClipperOptions *)clipAudio{
...
- (void)startClippingAudioWithCompletion:(void (^ _Nullable)(NSString*, CJGAVClipStatus))handler;
{
    ...
    if (!_clipAudio) {
        CJGLError(@"have not set a valid audio track");
 ...
 * @param handler block
 */
- (void)exportAudioWithCompletionHandler:(void (^ _Nullable)(NSString*, CJGAVClipStatus))handler;
{
    ...
    if (!insertResult) {
        CJGLError(@"mix insert error : %@",error);
    }
    ...
    if ((_deleteTimeRangeArr && _deleteTimeRangeArr.count > 0) && CMTimeCompare(_clipAudio.atTimeRange.duration, _clipAudio.mediaTimeRange.duration) == 0) {
        ...
        //更新时间对象
        _clipAudio.atTimeRange = [CJGAVTimeRange timeRangeWithStartTime:kCMTimeZero duration:mediaTotalTime];
    ...
    [_exporter exportAsynchronouslyWithCompletionHandler:^{
        CJGAVClipStatus exportStatus = CJGAVClipStatusUnknown;
        ...
        if (self->_exporter.error) {
            CJGLError(@"exporter audio error : %@",self->_exporter.error);
        }
...
- (void)resetClipperOperation;
{
    if (_exporter.status == AVAssetExportSessionStatusExporting || _exporter.status == AVAssetExportSessionStatusWaiting) {
        CJGLWarn(@"Conditions cannot be reset during operation.");
        if (_exporter) {
