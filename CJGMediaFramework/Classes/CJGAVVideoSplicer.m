...
@interface CJGAVVideoSplicer ()<CJGAVAssetExportSessionDelegate>
...
// 输出的 session
@property (nonatomic,strong) CJGAVAssetExportSession  *exporter;
// 视频 size
...
@implementation CJGAVVideoSplicer

- (CJGAVVideoSplicerOptions *)spliceOptions;
{
    if (!_spliceOptions) {
        
        _spliceOptions = [[CJGAVVideoSplicerOptions alloc] init];
    }
...
- (void)startSplicingWithCompletionHandler:(void (^ _Nullable)(NSString *filePath, CJGAVSpliceStatus status))handler;
{
    ...
    {
        CJGLError(@"CJGAVVideoSplicer: Invalid movies");
        [self notifyStatus:CJGAVSpliceStatusFailed];
        return;
    }
...
- (void)cancelSplicing;
{
    [self notifyStatus:CJGAVSpliceStatusCancelled];
...
/**
 ...
 @param videos 多视频数组
 */
- (void)spliceVideos:(NSArray <CJGAVVideoSplicerOptions *> *)videos{
    if(!_videos) _videos = videos;
    
    //遍历处理多视频
    [videos enumerateObjectsUsingBlock:^(CJGAVVideoSplicerOptions * _Nonnull video, NSUInteger i, BOOL * _Nonnull stop) {
...
/**
 ...
 @param handler 回调
 */
- (void)exportAsynchronouslyWithVideoCompostion:(AVMutableVideoComposition *)videoCompostion completionHandler:(void (^ _Nullable)(NSString *filePath, CJGAVSpliceStatus status))handler;
{
    // 创建一个可变的音频混合对象
    ...
    if (!_exporter) {
        _exporter = [[CJGAVAssetExportSession alloc] initWithAsset:self.spliceComposition];
    ...
    //正在合成通知回调
    [self notifyStatus:CJGAVSpliceStatusMerging];
    ...
    [_exporter exportAsynchronouslyWithCompletionHandler:^{
        ...
        CJGAVSpliceStatus exportStatus = CJGAVSpliceStatusUnknown;
        switch (self.exporter.status)
        {
            case AVAssetExportSessionStatusUnknown:
                exportStatus = CJGAVSpliceStatusUnknown;
                break;
            case AVAssetExportSessionStatusExporting:
                exportStatus = CJGAVSpliceStatusMerging;
                break;
            case AVAssetExportSessionStatusCompleted:
                exportStatus = CJGAVSpliceStatusCompleted;
                break;
            case AVAssetExportSessionStatusFailed:
                exportStatus = CJGAVSpliceStatusFailed;
                break;
            case AVAssetExportSessionStatusCancelled:
                exportStatus = CJGAVSpliceStatusCancelled;
                break;
            default:
                exportStatus = CJGAVSpliceStatusFailed;
                break;
...
/**
 ...
 @return 返回是否添加成功
 */
- (BOOL)addAudioTrackWithVideo:(CJGAVVideoSplicerOptions *)video atIndex:(NSInteger)index;
{
    ...
    if (video.enableVideoSound)
    {
        ...
        if (audioTracks.count > 0) {
            ...
            if (errorAudio)  CJGLError(@"Error while add audio track (%d): %@",index,errorAudio);
...
/**
 ...
 @return 返回是否添加成功
 */
- (BOOL)addVideoTrackWithVideo:(CJGAVVideoSplicerOptions *)video atIndex:(NSInteger)index;
{
    ...
    if (videoTracks.count > 0) {
        ...
        if (errorVideo)  CJGLError(@"Error while add video track (%d): %@",index,errorVideo);
        ...
        CJGLError(@"This video has no video tracks.");
        return NO;
    }
}
...
- (void)resetSplicerOperation;
{
    if (_exporter.status == AVAssetExportSessionStatusExporting || _exporter.status == AVAssetExportSessionStatusWaiting) {
        CJGLError(@"Conditions cannot be reset during operation.");
        if (_exporter) {
 ...
 */
- (void)notifyStatus:(CJGAVSpliceStatus)status;
{
    ...
    CJGRunSynchronouslyOnMainQueue(^{
        ...
        if (status == CJGAVSpliceStatusCompleted) {
