...
@interface CJGAVH246VideoDecoder ()
{
...
@implementation CJGAVH246VideoDecoder
...
- (instancetype)init
{
    ...
    if (self) {
        ...
        _bufferShowType = CJGAVH246VideoDecoderBufferShowType_Image;
        //创建了一个队列, 用于解码数据
...
- (CADisplayLink *)dispalyLink{
    ...
    if (!_dispalyLink) {
        //消息转发，用了解决循环引用
        CJGProxy *proxy = [CJGProxy proxyWithTarget:self];
...
- (void)initBufferDisplayLayer{
    ...
    if (_bufferShowType == CJGAVH246VideoDecoderBufferShowType_Layer) {
    ...
    dispatch_sync(_decodeQueue, ^{
        ...
        if (self->packetBuffer == NULL || self->packetSize == 0) {
            ...
            if (self.decodeDelegate && [self.decodeDelegate respondsToSelector:@selector(didChangedVideoDecodeState:videoDecoder:)]) {
                [self.decodeDelegate didChangedVideoDecodeState:CJGAVH246VideoDecoderStateFinish videoDecoder:self];
            }
...
static void decompressionOutputCallback(void *decompressionOutputRefCon, void *sourceFrameRefCon, OSStatus status, VTDecodeInfoFlags infoFlags, CVImageBufferRef pixelBuffer, CMTime presentationTimeStamp, CMTime presentationDuration) {
    ...
    *outputPixelBuffer = CVPixelBufferRetain(pixelBuffer);
    ...
    CJGAVH246VideoDecoder *decoder = (__bridge CJGAVH246VideoDecoder *)decompressionOutputRefCon;
...
- (void)initVideoDecopressionSession{
    ...
    if (self.decodeDelegate && [self.decodeDelegate respondsToSelector:@selector(didChangedVideoDecodeState:videoDecoder:)]) {
        [self.decodeDelegate didChangedVideoDecodeState:CJGAVH246VideoDecoderStateStart videoDecoder:self];
    }
}
...
- (CVPixelBufferRef)decodeVideoStreamingDataBufferFrame:(uint8_t *)frame bufferFrameSize:(long)frameSize{
    ...
    */
    ...
    if (_bufferShowType == CJGAVH246VideoDecoderBufferShowType_Layer) {//直接将sampleBuffer传给AVSampleBufferDisplayLayer来显示
        //记录sampleBuffer值
        ...
        if (_bufferShowType == CJGAVH246VideoDecoderBufferShowType_Image) {
    ...
    if (self.decodeDelegate && [self.decodeDelegate respondsToSelector:@selector(didChangedVideoDecodeState:videoDecoder:)]) {
        [self.decodeDelegate didChangedVideoDecodeState:CJGAVH246VideoDecoderStateDecoding videoDecoder:self];
    }
...
- (UIImage *)snapshot
{
    ...
    if (self.bufferDisplayLayer) {
        ...
        if (_bufferShowType == CJGAVH246VideoDecoderBufferShowType_Pixel) {
            if (self.pixelBuffer) {
