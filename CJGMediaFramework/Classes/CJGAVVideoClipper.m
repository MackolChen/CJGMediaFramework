...
@interface CJGAVVideoClipper ()<CJGAVAssetExportSessionDelegate>
{
    //音轨混合数组
    ...
    // 回调
    void (^_handler)(NSString *filePath, CJGAVClipStatus status);
    // 是否需要手动调用start
...
@implementation CJGAVVideoClipper
...
- (void)setClipVideo:(CJGAVVideoClipperOptions *)clipVideo{
...
- (void)startClippingVideoWithCompletion:(void (^ _Nullable)(NSString *filePath, CJGAVClipStatus status))handler;
{
    ...
    if (_clipVideo.clipStatus == CJGAVClipStatusClipping) {
        [self cancelClipping];
        _handler = handler;
    ...
    if (!_clipVideo.mediaURL) {
        ...
        CJGLError(@"Videopath is nil, please choose a video file to mix");
        [self notifyStatus:CJGAVClipStatusFailed];
        return;
    }
...
- (AVMutableVideoComposition *)processMediaOfVideoTrack{
    ...
    if (_enableVideoSound) {
        ...
        if (audioTrackArr.count > 0) {
            ...
            CJGLError(@"This video has no audio tracks.");
        }
    }
 ...
 */
- (void)exportClippedVideoWithVideoComposition:(AVMutableVideoComposition *)mainComposition completionHandler:(void (^)(NSString* outputFilePath, CJGAVClipStatus status))handler;
{
    ...
    if ((_deleteTimeRangeArr && _deleteTimeRangeArr.count > 0) && CMTimeCompare(_clipVideo.atTimeRange.duration, _clipVideo.mediaTimeRange.duration) == 0) {
        ...
        //更新时间对象
        _clipVideo.atTimeRange = [CJGAVTimeRange timeRangeWithStartTime:kCMTimeZero duration:mediaTotalTime];
    ...
    // 输出
    if (!_exporter) {
        _exporter = [[CJGAVAssetExportSession alloc] initWithAsset:_clipComposition];
    ...
    [self notifyStatus:CJGAVClipStatusClipping];
    ...
    [_exporter exportAsynchronouslyWithCompletionHandler:^{
        ...
        CJGAVClipStatus exportStatus = CJGAVClipStatusFailed;
        switch (self->_exporter.status)
        {
            case AVAssetExportSessionStatusUnknown:
                exportStatus = CJGAVClipStatusFailed;
                break;
            case AVAssetExportSessionStatusExporting:
                exportStatus = CJGAVClipStatusClipping;
                break;
            case AVAssetExportSessionStatusCompleted:
                exportStatus = CJGAVClipStatusCompleted;
                break;
            case AVAssetExportSessionStatusFailed:
                exportStatus = CJGAVClipStatusFailed;
                break;
            case AVAssetExportSessionStatusCancelled:
                exportStatus = CJGAVClipStatusCancelled;
                break;
            default:
                exportStatus = CJGAVClipStatusFailed;
                break;
...
- (void)cancelClipping;
{
    [self notifyStatus:CJGAVClipStatusCancelled];
    [self resetClipperOperation];
...
- (CMTime)outputLastTimeOfVideoClipWithTotolTime:(CMTime)totalTime{
    ...
    if (_deleteTimeRangeArr && _deleteTimeRangeArr.count > 0) {
        ...
        for (NSInteger i = _deleteTimeRangeArr.count - 1; i >= 0; i--) {
            CJGAVTimeRange *timeRange = _deleteTimeRangeArr[i];
 ...
 */
- (void)removeTimeRange:(CJGAVTimeRange *)timeRange;
{
    if (!timeRange.isValid) {//时间范围是无效的
        CJGLError(@"%@ timeRange is invalid",[timeRange description]);
        return;
    }
...
/**
 ...
 @return 排序之后的时间片段数组
 */
- (NSArray<CJGAVTimeRange *> *)bubbleSort:(NSMutableArray<CJGAVTimeRange *> *)sortArr;
{
...
- (NSArray<CJGAVTimeRange *> *)quickSort:(NSArray<CJGAVTimeRange *> *)sortArr{
    
    NSMutableArray<CJGAVTimeRange *> * resultArr = [NSMutableArray arrayWithArray:sortArr];
    NSInteger first = 0;
...
/**
 ...
 @param last  末索引
 */
- (void)quickSort:(NSMutableArray<CJGAVTimeRange *> *)sortArr withFirst:(NSInteger)first withLast:(NSInteger)last{
    ...
    NSInteger j = last;
    //标兵值(关键值)
    CJGAVTimeRange * keyTime = sortArr[i];
    //查询
    while (i<j) {
 ...
 */
- (void)notifyStatus:(CJGAVClipStatus)status;
{
